...
export class AppComponent {
  ...
  // Social profiles
  socialProfiles: { platform: string, url: string, icon: string }[] = [];
  newSocialPlatform = '';
  newSocialUrl = '';
  ...
  onFileSelected(event: any): void {
    ...
    if (file) {
      ...
      reader.onload = (e: any) => {
        this.signature.profileImage = e.target.result;
      };
  ...
  addSocialProfile(): void {
    ...
    if (!this.newSocialUrl.startsWith('http://') && !this.newSocialUrl.startsWith('https://')) {
      this.newSocialUrl = 'https://' + this.newSocialUrl;
    }

    if (!urlPattern.test(this.newSocialUrl)) {
    ...
    if (this.newSocialPlatform && this.newSocialUrl) {
      ...
      // Simple mapping for popular platforms
      switch(this.newSocialPlatform.toLowerCase()) {
        case 'linkedin':
        ...
        case 'whatsapp':
          ...
          break;
        default:
          icon = 'M8 0C3.58 0 0 3.58 0 8s3.58 8 8 8 8-3.58 8-8-3.58-8-8-8zm0 14.5c-3.58 0-6.5-2.92-6.5-6.5S4.42 1.5 8 1.5s6.5 2.92 6.5 6.5-2.92 6.5-6.5 6.5z';
      }
  ...
  async copyToClipboard(): Promise<void> {
    ...
    if (signatureElement) {
      try {
        ...
        // Create a Blob with both HTML and text content
